...
\]
其中 \(L_{\mathrm{new}} = L_{\mathrm{prompt}} - L_{\mathrm{reuse}}\) 表示未能复用、仍需重新 prefill 的 token 数。因此，缓存收益不只取决于是否启用缓存，更取决于跨轮 prompt 是否拥有足够长且完全一致的前缀。这一前缀复用视角与 Prompt Cache、SGLang/RadixAttention 以及当前 API prompt caching 的实践说明一致~\cite{gim2024promptcache,zheng2024sglang,openai2026promptcaching}。
